...
function measureLabelSize(ctx, options) {
  ...
  const fonts = isArray(optFont) ? optFont.map((f: any) => toFont(f)) : [toFont(optFont)];
...
function retrieveScaleID(scales, options, key) {
  ...
  const axes: any = Object.values(scales).filter((scale: any) => scale.axis && scale.axis === axis);
  if (axes.length) {
    return axes[0].id;
  }
  return axis;
...
function resolvePointProperties(chart, options) {
  if (!isBoundToPoint(options)) {
    const box: any = resolveBoxProperties(chart, options);
...
function resolveBoxAndLabelProperties(chart, options) {
  const properties: any = resolveBoxProperties(chart, options);
...
function resolveLimitedLineProperties(scales, area, options) {
  for (const scaleId of Object.keys(limitedLineScale)) {
    ...
    if (scale) {
      ...
      const dim: any = getDimensionByScale(scale, {min: options[min], max: options[max], start: scale[start], end: scale[end]});
...
class BoxAnnotation extends Element {
  ...
  inRange(mouseX, mouseY, axis, useFinalPosition) {
    ...
    return inBoxRange({x, y}, (this.getProps as any)(['x', 'y', 'x2', 'y2'], useFinalPosition), axis, this.options.borderWidth);
  }
  ...
  draw(ctx) {
    ctx.save();
    translate(ctx, (this.getCenterPoint as any)(), this.options.rotation);
    drawBox(ctx, this, this.options);
    ctx.restore();
  }

  get label() {
    return (this as any).elements && (this as any).elements[0];
  }
...
(BoxAnnotation as any).id = 'boxAnnotation';
...
(BoxAnnotation as any).descriptors = {
  label: {
    _fallback: true
  }
};
...
class LabelAnnotation extends Element {
  ...
  inRange(mouseX, mouseY, axis, useFinalPosition) {
    const {x, y} = rotated({x: mouseX, y: mouseY}, this.getCenterPoint(useFinalPosition), toRadians(-(this as any).rotation));
    return inBoxRange({x, y}, (this.getProps as any)(['x', 'y', 'x2', 'y2'], useFinalPosition), axis, this.options.borderWidth);
  }
  ...
  draw(ctx) {
    ...
    const visible = !defined((this as any)._visible) || (this as any)._visible;
    if (!options.display || !options.content || !visible) {
      return;
    }
    ctx.save();
    translate(ctx, (this as any).getCenterPoint(), (this as any).rotation);
    drawCallout(ctx, this);
    drawBox(ctx, this, options);
    drawLabel(ctx, getLabelSize((this as any)), options);
    ctx.restore();
  }
...
(LabelAnnotation as any).id = 'labelAnnotation';
...
function resolveCalloutPosition(element, options) {
  const position: any = options.position;
...
console.log(angleInCurve)
console.log(coordAngleInCurve)
